...
// import { getDisplayStatus } from "/src/utils/getDisplayStatus.js";
...
//       const status = getDisplayStatus(dayBookings);
...
//     const nextMonthDays = [];
//     const filled = [...prevMonthDays, ...currentMonthDays].length;
...
// function Calendar({ bookings = [], userId, onDayClick, onMonthChange }) {
//   const [month, setMonth] = useState(new Date());
//   const [days, setDays] = useState([]);
...
//       const bookedByUser = dayBookings.some((b) => b.userId === userId);
...
//                 isDisabled ? "disabled" : ""
//               }`}
//               onClick={() => !isDisabled && onDayClick?.(d.date)}
//               title={
//                 d.bookedByUser
//                   ? "Tu reserva"
//                   : d.status === "full"
...
function Calendar({ bookings = [], onDayClick, onMonthChange }) {
  ...
  const generateCalendar = () => {
    console.log("Generating calendar for month", month.toLocaleString("es-ES", { month: "long", year: "numeric" }));
    const year = month.getFullYear();
    const monthIndex = month.getMonth();
